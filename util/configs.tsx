...
function resultScreens(
  solendAction: SolendActionCore,
  setResult: (result: ResultConfigType | null) => void,
) {
  const totalTxns =
    Number(Boolean(solendAction.preTxnIxs.length)) +
  ...
  return {
    ...
    lendingCallback: () =>
      setResult({
        ...
        message: (
          <>
            ({1 + Number(Boolean(solendAction.preTxnIxs.length))}/{totalTxns})
            Lending transaction sent
            {'\n'}
            Waiting for confirmation...
          </>
        ),
        ...
        message: (
          <>
            ...
            {1 +
              Number(Boolean(solendAction.lendingIxs.length)) +
              Number(Boolean(solendAction.preTxnIxs.length))}
            /{totalTxns}) Cleanup transaction sent
            {'\n'}
            Waiting for confirmation...
          </>
        ),
...
export const supplyConfigs: ActionConfig = {
  ...
  verifyAction: (
    ...
    const positionLimitReached =
      ...
      !obligation.deposits.map(d => d.reserveAddress).includes(reserve.address);
...
export const borrowConfigs: ActionConfig = {
  action: async (
    ...
    const {preCallback, lendingCallback, postCallback} = resultScreens(
      solendAction,
      setResult,
    );
  ...
  verifyAction: (
    ...
    const positionLimitReached =
      obligation.positions > POSITION_LIMIT &&
      !obligation.borrows.map(d => d.reserveAddress).includes(reserve.address);
