...
function resultScreens(solendAction: SolendActionCore, setResult: (result: ResultConfigType | null) => void) {
      const totalTxns = Number(Boolean(solendAction.preTxnIxs.length)) +
        Number(Boolean(solendAction.lendingIxs.length)) +
        Number(Boolean(solendAction.postTxnIxs.length))
  
        return {
            preCallback: () => setResult({
            type: 'loading',
            message: <>
            {totalTxns} total transactions will have to be sent, requiring
            multiple approvals.
            {"\n"}
            {"\n"}(1/{totalTxns}) Setup transaction sent
            {"\n"}
            Waiting for confirmation...
          </>
        }),
        lendingCallback: () => setResult({
            type: 'loading',
            message: <>
            ({1 + Number(Boolean(solendAction.preTxnIxs.length))}/{totalTxns}) Lending transaction sent
            {"\n"}
            Waiting for confirmation...
          </>
        }),
        
        postCallback: () => setResult({
            type: 'loading',
            message: <>
            ({1  + Number(Boolean(solendAction.lendingIxs.length)) + Number(Boolean(solendAction.preTxnIxs.length))}/{totalTxns}) Cleanup transaction sent
            {"\n"}
            Waiting for confirmation...
          </>
        }),
  ...
  export type ActionConfig = {
    ...
    calculateMax: (
        ...
        _wallet: WalletType,
        obligation: ObligationType | null,
        rateLimiter: ParsedRateLimiter | null,) => BigNumber,
  }
  
  export const supplyConfigs: ActionConfig = {
    action: async (
      value: string,
      publicKey: string,
      pool: PoolType,
  ...
  export const borrowConfigs: ActionConfig = {
    action: async (
      value: string,
      publicKey: string,
      pool: PoolType,
      ...
      const {preCallback, lendingCallback, postCallback} = resultScreens(solendAction, setResult);
  ...
  export const withdrawConfigs: ActionConfig = {
    action: async (
      value: string,
      publicKey: string,
      pool: PoolType,
  ...
  export const repayConfigs: ActionConfig = {
    action: async (
      value: string,
      publicKey: string,
      pool: PoolType,
